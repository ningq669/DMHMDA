...
class DMHMDA(nn.Module):
    def __init__(self, param, m_emd, d_emd):
        super(DMHMDA, self).__init__()
        ...
        self.DMHM = HAN_DTI(self.all_meta_paths, self.inSize, self.hiddenSize, self.outSize, self.Dropout).to(
            self.device)
        self.HHN = HHN(param)
        self.args = param
        self.Xm = m_emd
    ...
    def forward(self, sim_data, train_data):
        ...
        h_11, h_21 = self.DMHM(self.graph, Em, Ed)
